...
print("HI HIHI")
#@jit
def solve_field(freq, zs, zr, z_ss, rp_ss, cw, z_sb, rp_sb, cb, rhob, attn, rbzb, dr, dz):

    # Check input dims and get flags indicating which model aspects are range-dependent
    rd_ss, rd_sb, rd_bt= check_inputs(zs, zr, z_ss, rp_ss, cw, z_sb, rp_sb, cb, rhob, attn, rbzb)

    # get model parameters (num pade coefficients, approximate wavelength, max plot depth, ...
    np, c0, lam, zmplt, rmax, ns, rs, lyrw, run_id = get_params(freq, zs, zr, z_ss, rp_ss, cw, z_sb, rp_sb, cb, rhob, attn, rbzb, dr, dz)

    ndr, ndz = 1, 1 # stride for saving outputs


    u, v, ksq, ksqw, ksqb,  alpw, alpb, p_rhob, r1, r2, r3, s1, s2, s3, f1, f2, f3, pd1, pd2, vr, vz, tll, tlg, cpl, cpg, tlc, ss_ind, sb_ind, bt_ind, zmax, nz, iz, nzplt, r, mdr, ir, ddir = setup(freq, zs, zr, z_ss, rp_ss, cw, z_sb, rp_sb, cb, rhob, attn, rbzb, dr, dz, ndr, ndz, np, c0, lam, zmplt, rmax, ns, rs, lyrw, run_id)
    ...
    for rn in range(nr):
        iz = updat(freq, zs, zr, z_ss, rp_ss, cw, z_sb, rp_sb, cb, rhob, attn, rbzb, dr, dz, zmax,
                                       np, c0, lam, zmplt, rmax, ns, rs, lyrw, run_id,
                                       u, v, ksq, ksqw, ksqb, alpw, alpb, p_rhob, r1, r2, r3, s1, s2, s3, f1, f2, f3, 
                                       pd1, pd2, vr, vz, tll, tlg, cpl, cpg, tlc,
                                       ss_ind, sb_ind, bt_ind, rd_ss, rd_sb, rd_bt,
                                       nz, iz, nzplt, r)
        ...
        mdr, tlc = \
            (outpt(r, mdr, ndr, ndz, tlc, f3,
                   u, ddir, ir, tll, tlg, cpl, cpg)[:])
...
#@njit
def get_params(freq, zs, zr, z_ss, rp_ss, cw, z_sb, rp_sb, cb, rhob, attn, rbzb, dr, dz):
    np_default = 8
    dzf = 0.1
    ns_default = 1
    lyrw_default = 20
...
#@njit
def setup(freq, zs, zr, z_ss, rp_ss, cw, z_sb, rp_sb, cb, rhob, attn, rbzb, dr, dz, ndr, ndz, np, c0, lam, zmplt, rmax, ns, rs, lyrw, run_id):
        '''Initialise the parameters, acoustic field, and matrices'''

        if rbzb[-1, 0] < rmax:
            rbzb = numpy.append(rbzb,
                                      numpy.array([[rmax, rbzb[-1, 1]]]),
                                      axis=0)
        ...
        ksqw, ksqb, p_rhob, alpw, alpb = profl(freq, c0, z_ss, rp_ss, cw, z_sb, rp_sb, cb, rhob, attn, rbzb,
              dr, dz, zmax, ss_ind, sb_ind, nz, lyrw, lam,
              ksqw, ksqb, alpw, alpb)

        # The self-starter
        selfs(freq, c0, zs, dz, dr, np, nz, iz, ns, u, v, s1, s2, s3, r1, r2, r3, f1, f2, f3,
                ksq, ksqw, ksqb, p_rhob, alpw, alpb, pd1, pd2)
        ...
        matrc(k0, dz, iz, iz, nz, np,
              f1, f2, f3, ksq, alpw, alpb,
              ksqw, ksqb, p_rhob, r1, r2, r3,
              s1, s2, s3, pd1, pd2)
        return u, v, ksq, ksqw, ksqb,  alpw, alpb, p_rhob, r1, r2, r3, s1, s2, s3, f1, f2, f3, pd1, pd2, vr, vz, tll, tlg, cpl, cpg, tlc, ss_ind, sb_ind, bt_ind, zmax, nz, iz, nzplt, r, mdr, ir, ddir
...
#@njit
def profl(freq, c0, z_ss, rp_ss, cw, z_sb, rp_sb, cb, rhob, attn, rbzb, 
          dr, dz, zmax, ss_ind, sb_ind, nz, lyrw, lam,
          ksqw, ksqb, alpw, alpb):
    '''Set up the profiles'''
    ...
    p_cw = interp(z, z_ss, cw[:, ss_ind],
                           cw[0, ss_ind],
                           cw[-1, ss_ind])
    p_cb = interp(z, z_sb[:, sb_ind], cb[:, sb_ind],
                           cb[0, sb_ind],
                           cb[-1, sb_ind])
    p_rhob = interp(z, z_sb[:, sb_ind], rhob[:, sb_ind],
                             rhob[0, sb_ind],
                             rhob[-1, sb_ind])
    attnlyr = numpy.concatenate((attn[:, sb_ind],
                                 numpy.array([attn[-1, sb_ind], attnf])))
    zlyr = numpy.concatenate((z_sb[:, sb_ind],
                              numpy.array([z_sb[-1, sb_ind] + 0.75 * lyrw * lam,
                               z_sb[-1, sb_ind] + lyrw * lam])))
    p_attn = interp(z, zlyr, attnlyr,
                             attn[0, sb_ind],
                             attnf)
    ...
    for i in range(nz + 2):
        ...
        ksqb[i] = ((omega / p_cb[i]) *
                        (1 + 1j * eta * p_attn[i]))**2 - k0**2
        alpw[i] = numpy.sqrt(p_cw[i] / c0)
        alpb[i] = numpy.sqrt(p_rhob[i] * p_cb[i] / c0)
    return ksqw, ksqb, p_rhob, alpw, alpb

#@njit
def updat(freq, zs, zr, z_ss, rp_ss, cw, z_sb, rp_sb, cb, rhob, attn, rbzb, dr, dz, zmax,
          np, c0, lam, zmplt, rmax, ns, rs, lyrw, run_id,
          u, v, ksq, ksqw, ksqb, alpw, alpb, p_rhob,
          r1, r2, r3, s1, s2, s3, f1, f2, f3, 
          pd1, pd2, vr, vz, tll, tlg, cpl, cpg, tlc,
          ss_ind, sb_ind, bt_ind, rd_ss, rd_sb, rd_bt, 
          nz, iz, nzplt, r):
#
...
#@njit
def selfs(freq, c0, zs, dz, dr, np, nz, iz, ns, u, v, s1, s2, s3, r1, r2, r3, f1, f2, f3,
          ksq, ksqw, ksqb, p_rhob, alpw, alpb, pd1, pd2):
    ...
    u[_is] = (1 - dis) * numpy.sqrt(2 * numpy.pi / k0) / \
        (dz * alpw[_is])
    u[_is + 1] = dis * numpy.sqrt(2 * numpy.pi / k0) / \
        (dz * alpw[_is])
...
def epade(np, ip, k0, dr, ns, pd1, pd2):
    ...
    dg, dh1, dh2, dh3 = \
        deriv(n, sig, alp, dg, dh1, dh2, dh3, _bin, nu)
    for i in range(n):
        b[i] = dg[i + 1]
    for i in range(n):
        if 2 * i <= n - 1:
